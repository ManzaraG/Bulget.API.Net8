...
public sealed class AuthenticationController(ISender sender) : ControllerBase
{
    ...
    {
        var result = await sender.Send(new RegisterCommand(dto.Nom, dto.Email, dto.MotDePasse), cancellationToken);
        return Ok(result);
    }
    ...
    {
        var result = await sender.Send(new LoginCommand(dto.Email, dto.MotDePasse), cancellationToken);
        return Ok(result);
    }
}
